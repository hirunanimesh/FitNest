"use client"
import { useState } from "react"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Separator } from "@/components/ui/separator"
import { Label } from "@/components/ui/label"
import Link from "next/link"
import { AppLogo } from "@/components/AppLogo"
import { supabase } from "@/lib/supabase"
import { useRouter } from "next/navigation"

export default function SignupPage() {
  const router = useRouter()
  const [isLoading, setIsLoading] = useState(false)
  const [selectedRole, setSelectedRole] = useState("")

  const handleGoogleSignUp = async () => {
    if (!selectedRole) {
      alert("Please select your role first")
      return
    }

    try {
      setIsLoading(true)
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/auth/complete-profile?role=${selectedRole}`,
          queryParams: {
            access_type: 'offline',
            prompt: 'consent',
          },
        }
      })

      if (error) {
        console.error("Google sign-up error:", error)
        // Handle error
      }
    } catch (error) {
      console.error("Error initiating Google sign-up:", error)
    } finally {
      setIsLoading(false)
    }
  }

  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="absolute top-4 left-4">
        <AppLogo />
      </div>
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl">Join FitNest</CardTitle>
          <CardDescription>Create your account and start your fitness journey</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {/* Quick Google Sign Up */}
          <div className="space-y-4">
            <h3 className="text-sm font-medium text-center">Quick signup with Google</h3>
            
            {/* Role Selection for Google OAuth */}
            <div className="space-y-3">
              <Label>Choose your role:</Label>
              <div className="grid grid-cols-1 gap-2">
                <Button
                  type="button"
                  variant={selectedRole === "customer" ? "default" : "outline"}
                  className="w-full justify-start"
                  onClick={() => setSelectedRole("customer")}
                >
                  🏃‍♂️ Customer
                </Button>
                <Button
                  type="button"
                  variant={selectedRole === "trainer" ? "default" : "outline"}
                  className="w-full justify-start"
                  onClick={() => setSelectedRole("trainer")}
                >
                  💪 Trainer
                </Button>
                <Button
                  type="button"
                  variant={selectedRole === "gym" ? "default" : "outline"}
                  className="w-full justify-start"
                  onClick={() => setSelectedRole("gym")}
                >
                  🏋️‍♀️ Gym Owner
                </Button>
              </div>
            </div>

            <Button 
              onClick={handleGoogleSignUp}
              variant="outline" 
              className="w-full bg-transparent flex items-center justify-center gap-3"
              disabled={isLoading || !selectedRole}
            >
              <svg width="18" height="18" viewBox="0 0 24 24">
                <path
                  fill="#4285F4"
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                />
                <path
                  fill="#34A853"
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                />
                <path
                  fill="#FBBC05"
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                />
                <path
                  fill="#EA4335"
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                />
              </svg>
              {isLoading ? "Connecting with Google..." : "Sign up with Google"}
            </Button>
            <p className="text-xs text-center text-muted-foreground">
              Quick and easy signup - complete your profile after connecting
            </p>
          </div>

          <Separator />

          {/* Manual Registration Options */}
          <div className="space-y-3">
            <h3 className="text-sm font-medium text-center">Or create account manually as:</h3>
            
            <Link href="/auth/signup/user" className="block">
              <Button variant="outline" className="w-full">
                🏃‍♂️ Sign up as Customer
              </Button>
            </Link>
            
            <Link href="/auth/signup/trainer" className="block">
              <Button variant="outline" className="w-full">
                💪 Sign up as Trainer
              </Button>
            </Link>
            
            <Link href="/auth/signup/gym" className="block">
              <Button variant="outline" className="w-full">
                🏋️‍♀️ Sign up as Gym
              </Button>
            </Link>
          </div>

          <Separator />

          <div className="text-center">
            <p className="text-sm text-muted-foreground">
              Already have an account?{" "}
              <Link href="/auth/login" className="text-primary hover:underline">
                Sign in
              </Link>
            </p>
          </div>
        </CardContent>
      </Card>
    </div>
  )
}
